    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-xl shadow-sm">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-semibold text-gray-800">Edit Kecamatan: {{ $kecamatan->nama_kecamatan }}</h2>
            </div>

            <form action="{{ route('admin.kecamatan.update', $kecamatan) }}" method="POST" class="p-6" noValidate>
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" id="luas_wilayah" name="luas_wilayah"
                            value="{{ old('luas_wilayah', $kecamatan->luas_wilayah) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-lg">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="jumlah_penduduk" name="jumlah_penduduk"
                            value="{{ old('jumlah_penduduk', $kecamatan->jumlah_penduduk) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-lg">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-lg">{{ old('deskripsi', $kecamatan->deskripsi) }}</textarea>
                </div>

                <!-- Polygon Drawing Map -->
                <div class="mb-4">
                    <label class="block text-lg font-medium text-gray-700 mb-2">
                        <i class="fas fa-draw-polygon mr-2"></i>Edit Batas Wilayah (Polygon)
                    </label>
                    <p class="text-gray-500 text-lg mb-3">Edit polygon yang ada atau hapus dan gambar ulang. Gunakan tool
                        edit (icon pensil) untuk mengubah titik-titik polygon.</p>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" id="latitude" name="latitude"
                                value="{{ old('latitude', $kecamatan->latitude) }}" readonly
                        ...
                        <div>
                            ...
                            <input type="text" id="longitude" name="longitude"
                                value="{{ old('longitude', $kecamatan->longitude) }}" readonly
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-100 text-lg cursor-not-allowed">
                        </div>
                    </div>

                    <input type="hidden" name="geojson_data" id="geojson_data"
                        value="{{ old('geojson_data', $kecamatan->geojson_data ? json_encode($kecamatan->geojson_data) : '') }}">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="color" id="warna" name="warna"
                        value="{{ old('warna', $kecamatan->warna ?? '#3388ff') }}"
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1"
                            {{ old('is_active', $kecamatan->is_active) ? 'checked' : '' }}
